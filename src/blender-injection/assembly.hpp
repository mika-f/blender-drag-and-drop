...
namespace Injector
{
    ...
    using INJECT_ASM = struct
    {
        BYTE binary[25];
    };

    using INJECT_ASM2 = struct
    {
        BYTE binary[20];
    };
    ...
    {
        using functor = injector_asm::wrapper<FuncT>;
        if constexpr (false) functor::call(nullptr); // To instantiate the template, if not done _asm will fail
