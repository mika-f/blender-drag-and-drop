...
// Code is stolen from https://github.com/matanki-saito/EU4dll
...
namespace Injector
{
    ...
    public:
        ...
        struct fn_mem_translator_aslr
        {
            ...
            {
                static uintptr_t module = (uintptr_t)GetModuleHandle(nullptr);
    ...
    public:
        ...
        template <class T>
        T* get() const { return static_cast<T*>(this->p); }

        template <class T>
        T* get_raw() const { return static_cast<T*>(this->p); }
    ...
    public:
        ...
        template <class T>
        basic_memory_pointer(T* x) : p(static_cast<void*>(x))
        {
        }
    ...
    using memory_pointer = basic_memory_pointer<address_manager::fn_mem_translator>;
    using memory_pointer_raw = basic_memory_pointer<address_manager::fn_mem_translator_nop>;
    using memory_pointer_aslr = basic_memory_pointer<address_manager::fn_mem_translator_aslr>;
     ...
     */
    template <class T>
    T& WriteObject(memory_pointer_tr addr, const T& value, bool vp = false)
    {
     ...
     */
    template <class T>
    T& ReadObject(memory_pointer_tr addr, T& value, bool vp = false)
    {
        scoped_unprotect xprotect(addr, vp ? sizeof(value) : 0);
     ...
     */
    template <class T>
    memory_pointer_tr WriteMemory(memory_pointer_tr addr, T value, bool vp = false)
    {
     ...
     */
    template <class T>
    T ReadMemory(memory_pointer_tr addr, bool vp = false)
    {
        T value;
    ...
    inline memory_pointer_raw MakeCALL(memory_pointer_tr at, memory_pointer_raw dest, bool vp = true)
    {
        ...
        auto aOffset = abs(static_cast<long long>(offset));
    ...
    {
        return MakeNOP(at, static_cast<size_t>(until.get_raw<char>() - at.get_raw<char>()), vp);
    }
    ...
    */
    ...
    template <class T>
    memory_pointer mem_ptr(T p)
    {
        return memory_pointer(p);
    }

    template <class T>
    memory_pointer_raw raw_ptr(T p)
    {
        return memory_pointer_raw(p);
    }

    template <class Tr>
    memory_pointer_raw raw_ptr(basic_memory_pointer<Tr> p)
    {
        return raw_ptr(p.get());
    }

    template <uintptr_t addr>
    memory_pointer_raw lazy_ptr()
    {
        return lazy_pointer<addr>::get();
    }

    template <class T>
    memory_pointer_aslr aslr_ptr(T p)
    {
