...
pair<uint8_t, uint8_t> BytePattern::parse_sub_pattern(std::string_view sub)
{
...
void BytePattern::transform_pattern(std::string_view literal)
{
    ...
    std::stringstream ss{_literal};
    std::string buffer;
    while (std::getline(ss, buffer, ' '))
        sub_patterns.push_back(buffer);

    // std::algorithm::split(sub_patterns, _literal, std::is_any_of(" "));
...
void BytePattern::get_module_ranges(memory_pointer module)
{
    ...
    for (int i = 0; i < ntHeader->FileHeader.NumberOfSections; i++)
    {
        ...
        if (memcmp(sec->Name, ".text", 6) == 0 || memcmp(sec->Name, ".rdata", 7) == 0)
        {
...
void BytePattern::bm_search()
{
    ...
    {
        auto range_begin = reinterpret_cast<uint8_t*>(range.first);
        auto range_end = reinterpret_cast<uint8_t*>(range.second - pattern_len);
